...
RUN echo "* * * * * /usr/local/bin/php -c /usr/local/etc/php/php.ini /opt/una/periodic/cron.php 2>&1 | sed -e \"s/\(.*\)/[\`date\`] \1/\" >>/opt/una/logs/crontab.log" > /tmp/crontab && crontab /tmp/crontab && rm -f /tmp/crontab
